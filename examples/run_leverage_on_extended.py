...
def main() -> None:
    closes = load_closes()
    if not closes:
        raise SystemExit("No data loaded; run examples.download_sweden_extended.py first.")

    bh = BuyAndHold()
    # single sleeve: 90% core, 4% dip, 10x overlay
    lev_single = LeverageDip(dip_pct=0.04, base_weight=0.9, leverage=10.0)
    # split sleeve: 90% core, 5% @10x; if that sleeve -50%, add 5% @20x
    ...
    print("=== Buy & Hold baseline (equal-weight across extended universe) ===")
    ...
    print("=== Split-sleeve leverage dip (90% core, 5% @10x, DD 50% then 5% @20x) ===")
